...
- while mixture component weights and parameters are often estimated as a by-product, all methods contained in **mixComp** are based on theory specifically developed to consistently estimate mixture complexity; 
...
The estimation procedure is based on initially finding the 'best' possible estimate $(\hat{\mathbf{w}}^j, \hat{\mathbf{\theta}}^j) \in W_j \times \Theta_j$ for a given value of $j$, in order to compare the thereby specified probability density/mass function 
