## tests for hellinger.boot.disc function:    
...
testthat::test_that("errors", {
  ...
  testthat::expect_error(
    hellinger.boot.disc(pois.dM), "'theta.bound.list' has to be specified as a datMix object attribute!")  
  ...
  testthat::expect_error(
    hellinger.boot.disc(c(1, 2, 3, 4)))
  
  ## input from a continuous distribution:
  testthat::expect_error(
    hellinger.boot.disc(normLoc.dM), "The 'discrete' attribute of the Rdat object is not set to TRUE, however
...
testthat::test_that("function checks", {    
  ...
  set.seed(0)
  res <- hellinger.boot.disc(pois.dM)
  ...
  set.seed(0)
  res <- hellinger.boot.disc(pois.dM, B = 50, ql = 0.1, qu = 0.9)
