## tests for L2.boot.disc function:    
...
testthat::test_that("errors", {
  ...
  testthat::expect_error(
    L2.boot.disc(pois.dM), "'theta.bound.list' has to be specified as a datMix object attribute!")  
  ...
  testthat::expect_error(
    L2.boot.disc(c(1, 2, 3, 4)))
  
  ## input from a continuous distribution:
  testthat::expect_error(
    L2.boot.disc(normLoc.dM), "The 'discrete' attribute of the Rdat object is not set to TRUE, however
...
testthat::test_that("function checks", {    
  ...
  # overwrite default maximum number of components and default quantiles:
  set.seed(0)
  res <- L2.boot.disc(pois.dM, ql = 0.1, qu = 0.9)
